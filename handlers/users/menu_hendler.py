...
from keyboards.inline.menu import categoryMenu,xizmatlar
from loader import dp


@dp.message_handler(text="avtoservis")
async def select_category(message: Message):
    logging.info(message)
    logging.info(f"{message.from_user.username=}")
    logging.info(f"{message.from_user.full_name=}")

    await message.answer(f"Xizmat turini tanlang", reply_markup=xizmatlar)

# @dp.message_handler(text_contains="avtoservis")
# async def select_category(message: Message):
#     logging.info(message)
#     logging.info(f"{message.from_user.username=}")
#     logging.info(f"{message.from_user.full_name=}")
...
#     await call.message.answer(f"Siz Django kursini tanladingiz.",
#                               reply_markup=django_keyboard)
#     await call.answer(cache_time=60)
